...
function countVowels(str) {
  return str
    .split('')
    .reduce((count, item) => count + 'aeiouyAEIOUY'.includes(item), 0);
}
...
function isPalindrome(str) {
  const resultString = str.replace(/[^a-zA-Zа-яА-Я]/g, '').toLocaleLowerCase();
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((max, item) => {
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((item) => {
      if (original.includes(item)) return encoded[original.indexOf(item)];
      return item;
    })
    .join('');
}
